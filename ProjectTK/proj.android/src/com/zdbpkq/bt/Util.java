...
public class Util {
	...
	private static void saveConfig() {
		synchronized (sync) {
			try {
				SharedPreferences preferences = ProjectMB.getContext()
						.getSharedPreferences("userconfing",
	...
	private static void loadConfig() {
		synchronized (sync) {
			...
			firstTimeInstall = preferences
					.getBoolean("firstTimeInstall", false);
		}
	}
	...
	public static void createShortCut() {
		...
		if (!Util.firstTimeInstall) {
			Util.firstTimeInstall = true;
			Util.saveConfig();
	...
	private static void createShortCutInternal() {
		...
		// 快捷方式名称
		Resources res = ProjectMB.getContext().getResources();
		int appname_id = res.getIdentifier("app_name", "string", ProjectMB
				.getContext().getPackageName());
		String app_name = ProjectMB.getContext().getString(appname_id);
		shortcutIntent.putExtra(Intent.EXTRA_SHORTCUT_NAME, app_name);
		// 快捷图片
		int icon_id = res.getIdentifier("icon", "drawable", ProjectMB
				.getContext().getPackageName());
		Parcelable icon = Intent.ShortcutIconResource.fromContext(
				ProjectMB.getContext(), icon_id);
		shortcutIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE, icon);
		// 点击快捷图片，运行的程序主入口
		shortcutIntent.putExtra(Intent.EXTRA_SHORTCUT_INTENT, new Intent(
				ProjectMB.getContext(), ProjectMB.class));
		ProjectMB.getContext().sendBroadcast(shortcutIntent);
	}

	public static void setupPushService() {
		// 开启logcat输出，方便debug，发布时请关闭
		XGPushConfig.enableDebug(ProjectMB.getContext(), false);
		// 如果需要知道注册是否成功，请使用registerPush(getApplicationContext(),
		// XGIOperateCallback)带callback版本
		// 如果需要绑定账号，请使用registerPush(getApplicationContext(),account)版本
		// 具体可参考详细的开发指南
		// 传递的参数为ApplicationContext
		XGPushManager.registerPush(ProjectMB.getContext(),
				new XGIOperateCallback() {

					@Override
					public void onFail(Object data, int errCode, String msg) {
						Log.d("TPush", "推送服务注册失败，错误码：" + errCode + ",错误信息："
								+ msg);
					}

					@Override
					public void onSuccess(Object data, int flag) {
						Log.d("TPush", "推送服务注册成功，设备token为：" + data);
					}
				});

		// 2.36（不包括）之前的版本需要调用以下2行代码
		Intent service = new Intent(ProjectMB.getContext(), XGPushService.class);
		ProjectMB.getContext().startService(service);
