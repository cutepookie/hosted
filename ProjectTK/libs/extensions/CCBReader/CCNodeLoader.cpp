            ...
            case kCCBPropTypeSpriteFrame: {
                ...
                if(setProp) 
                {
                    if (ccSpriteFrame == NULL)
                    {
                        ...
                        ccSpriteFrame = CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("notexture.png");
                    }
...
CCSpriteFrame * CCNodeLoader::parsePropTypeSpriteFrame(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader, const char *pPropertyName) 
{
    ...
    if (spriteFile.length() != 0)
    {
        ...
        if (spriteFrame == NULL)
        {
            CCLog("LoadSpriteError:%s,%s",spriteSheet.c_str(),spriteFile.c_str());
        }
    }
