//
//  MB_EquipBackpackDataSource.cpp
//  ProjectMB
//
...
MB_EquipBackpackDataSource::MB_EquipBackpackDataSource()
{
    isAllRun = true;
    m_pItemArray = NULL;
...
CCSize MB_EquipBackpackDataSource::tableCellSizeForIndex(CCTableView *table, unsigned int idx) {
    if (allCellCount-1==idx) {
        return m_szCell+CCSize(0,LASTCELLADDHEIGHT);
    }
    return m_szCell;
};
CCTableViewCell* MB_EquipBackpackDataSource::tableCellAtIndex(CCTableView *table, unsigned int idx)
{
...
unsigned int MB_EquipBackpackDataSource::numberOfCellsInTableView(CCTableView *table)
{
    if (m_pItemArray==NULL) {
        m_pItemArray = MB_ItemMgr::getInstance()->getEquipArray();
